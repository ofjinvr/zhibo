...
$(document).ready(function(e) {
	...
	$(".relationInfo dl dt .info .download,.relationInfo dl dt a.downData").click(function(e) {	
		...
		$.ajax({
			...
			success: function(data) {
				if(data.status == 1){
					window.location.href="/Wap/Video/downfile/id/"+sourceId+".html";
				}else{
	...
	$("#commentButton").click(function(e) {	
		...
		$.ajax({
			type: "post",
			url: "/Wap/Video/commentSubmit.html",
			async: false, //同步加载,默认为异步
			cache: false,
			...
			success: function(data) {
				if(data.status == 1){
					...
					window.location.href="/Wap/Video/videoDetail/id/"+vodeoId+"/comment/1.html?"+Math.random()+"#addComment";
				}else{
					window.art.dialog({content:data.info,lock:true,width:250,height:100}).time(3);
					return false;
				}
...
function addVideoScure(num){
	$.ajax({
		type: "post",
		url: "/Wap/Video/addVideoScore.html",
		async: false, //同步加载,默认为异步
		cache: false,
		data:"id="+num,
		dataType: "json",
